...
export function OptionsPage({ purchaseData, onBack }: OptionsPageProps) {
  ...
  const handleActionClick = (actionId: string) => {
    ...
    try {
      ...
      // NEW APPROACH: For refunds, use new EmailPromptDialog component
      if (actionId === 'refund') {
        console.log('🔥🔥🔥 REFUND ACTION - Using NEW EmailPromptDialog');
  ...
  try {
    return (
      ...
      <div className="container-grid max-w-6xl">
        ...
        {/* NEW: Email Prompt Dialog Component */}
        <EmailPromptDialog 
          ...
          onSubmit={(email) => {
            console.log('🔥🔥🔥 EMAIL RECEIVED FROM NEW DIALOG:', email);
            setUserEmail(email);
            handleConfirmAction();
          }}
        />
